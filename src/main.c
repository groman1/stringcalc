...
{
	if (argc==1) printf("Enter your equations, enter q when you are finished\n");
	for (int p = 1;argc==1?1:p<argc; ++p)
	{
		char input[200];
		if (argc>1) {strcpy(input, argv[p]);input[strlen(input)]='\n'; input[strlen(input)+1]='\0';}
		...
		{
			printf("%.10g\n", arr[lastaction]); //incase the action is		// r/redditsniper LOL
		}
		free(arr);
		free(act);
	}
