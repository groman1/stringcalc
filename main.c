...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < strlen(input)-1; ++i) //parser, ignoring the \n and \0
    {
        switch (input[i])
        {
            ...
            case '-': {act[ran].action = '-'; arr[currindex_num]/=digitsafterdecimal; digitsafterdecimal = 1; hasdecimal = 0; if(i==0||input[i-1]=='('){ act[ran].id1=-2;act[ran].id2 = currindex_num; } else {act[ran].id1 = currindex_num; act[ran].id2 = currindex_num+1;} act[ran].priority = 1; if (act[ran].id1!=-2) {++currindex_num; arr=realloc(arr, 8*(currindex_num+1)); arr[currindex_num] = 0;} ++ran; act=realloc(act, sizeof(action)*(ran+1)); break;}
        ...
        for (int i = 0; i <= ran; ++i) //search for 5+ priority (parentesis' searcher) 
        {
            if(act[i].priority==6+c&&act[i].action=='(') 
            {
                for (int x = 1;x < ran-i;++x) // negative numbers
                {
                    if (act[i+x].action==')')
                    { 
                        if (act[i-1].priority<act[i+x+1].priority) { act[i+x+1].id1 = act[i].id1; } 
                        else { act[i-1].id2 = act[i].id1; act[i+x+1].id1 = act[i-1].id1; } // if the action after the par. is higher priority
                        break;
    ...
    for (int i = 0; i < ran; ++i) //search for 5 priority exponentation outside of parenthesis'
    {
        ...
        if (act[i+1].priority<5)
        {
            act[i+1].id1 = act[i].id1;
        }
    ...
    for (int i = 0; i < currindex_num; ++i) // calculation
    {
        if(ran)
        {
            ...
            {           
                ...
                case '-': act[priority_list[i]].id1==-2?(arr[act[priority_list[i]].id2]=0-arr[act[priority_list[i]].id2], ++currindex_num):(arr[act[priority_list[i]].id1]-=arr[act[priority_list[i]].id2]);  break;
                case '*': arr[act[priority_list[i]].id1]*=arr[act[priority_list[i]].id2];  break;
